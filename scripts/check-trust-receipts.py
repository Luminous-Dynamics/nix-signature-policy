...
if hashlib.sha256(compact).hexdigest()!=example['payload_sha256']:
    raise SystemExit('trust receipt example has an invalid payload digest')
src=(ROOT/'src/receipt.rs').read_text(); cargo=(ROOT/'Cargo.toml').read_text(); doc=(ROOT/'docs/TRUST_RECEIPTS.md').read_text().lower()
for token in ['build_trust_receipt','verify_trust_receipt','canonical_policy_sha256','canonical_registry_sha256','qualifying_families','deny_unknown_fields']:
...
print('trust receipt schema and positioning checks passed')
