...
| `env_clear()`, no `PATH`, opt-in allowlist only | An absolute helper path needs no `PATH` to resolve; clearing it removes an entire class of environment-controlled lookup surface. Callers must configure an absolute executable path. |
